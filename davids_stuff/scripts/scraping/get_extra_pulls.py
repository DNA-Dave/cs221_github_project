...
with open("need_pull_requests.txt") as input_file:
    for line in input_file:
        to_download.append(line.rstrip().replace("_", "/", 1))
output = open("project_to_num_pulls_second_round.txt", "w+")
for i in tqdm(range(len(to_download))):
    element = to_download[i]
    url = "https://api.github.com/repos/" + element + "/pulls?state=all"

...
with open("need_contributors.txt") as input_file:
    for line in input_file:
        to_download.append(line.rstrip().replace("_", "/", 1))
output = open("project_to_contributors_second_round.txt", "w+")
for i in tqdm(range(len(to_download))):
    element = to_download[i]
    url = "https://api.github.com/repos/" + element + "/contributors"
